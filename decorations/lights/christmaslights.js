...
function XLSF(oTarget) {
  ...
  if (window.lightsize) {
    this.lightClass = window.lightsize;
  }
  ...
  this.destroyLight = function() {
    ...
    if (self.lightSmashCounter<self.lights.length) {
      ...
      while (self.numSmashed < limit && !except) {
        try {
          // self.lights[self.lightSmashCounter].smash();
          if (self.lights[self.currentSmashing].broken == false) {
            self.lights[self.currentSmashing].smash();
            self.numSmashed++;
          } else {
            logger("Skipping over " + self.currentSmashing + " (already smashed)");
          }
          self.currentSmashing++;
        } catch (e) {
          logger("Exception in destroyLight(): " + e);
          except = true;
        } // added by jake so we don't fail...just in case
      }
    } else {
      logger("End - stopping sequence");
  ...
  this.stopSequence = function(erase_ourselves) {
    if (self.timer) {
      window.clearInterval(self.timer);
      self.timer = null;
    }
    // NOTE TO SELF (jake): Added so we can run funcs after breaking
    if (erase_ourselves && typeof window.lightsBreakOverFunc == "function") {
        window.lightsBreakOverFunc();
    }
  }
